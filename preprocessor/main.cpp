...
// определяем свой литерал для пути filesystem::path
// вместо path("a") / path("folder") / path("a") / path("file.txt")
// можно писать "a"_p / "folder"_p / "a"_p / "file.txt"_p 
...
bool SubstitutionOfDirectives(istream& input, ostream& output, const path& file_name, const vector<path>& include_directories) {
    ...
    for (string str; getline(input, str); ) {
        ...
        if (regex_match(str, matched, local_reg)) {
            ...
            if (filesystem::exists(next_path)) {
                ...
                if (in.is_open()) {
                    if (!SubstitutionOfDirectives(in, output, next_path.string(), include_directories)) {
                        return false;
                    }
                    continue;
                }
...
bool Preprocess(const path& in_file, const path& out_file, const vector<path>& include_directories) {
    ...
    return SubstitutionOfDirectives(in, out, in_file, include_directories);
}
